...
class AppStore extends Component {
  state = {input: '', activeId: tabsList.tabId[0]}
  ...
  filterProjects = () => {
    ...
    const projects = appsList.filter(each => each.category === activeId)
    return projects
  }

  filteredApps = () => {
    const {input} = this.state
    const res = appsList.filter(eachPro =>
      eachPro.appName.toLowerCase().includes(input.toLowerCase()),
    )
    return res
  }

  render() {
    const {input, activeId} = this.state
    const res = this.filteredApps()
    const {projects} = this.filterProjects(res)
        ...
        <ul className="tab-list">
          {tabsList.map(eachTab => (
            <TabItem
              key={eachTab.tabId}
              tabDetails={eachTab}
              onClickTab={this.onSetTab}
              isActive={eachTab.tabId === activeId}
            />
          ))}
        </ul>
        <ul className="app-list">
          {projects.map(eachApp => (
            <AppItem key={eachApp.appId} details={eachApp} />
          ))}
        </ul>
      </div>
    )
